...
export async function GET() {
  ...
  const [
    ...
    svc
      ...
      .select(
        "monetization_on,price_text,pay_number,pay_instructions,pro_days,managed_ai_on",
      )
      .eq("id", 1)
      .maybeSingle(),
    ...
    svc
      ...
      .select("id,user_id,method,trx_id,status,created_at")
      .order("created_at", { ascending: false })
      .limit(100),

    svc
      .from("customers")
      .select("user_id,last_msg_at")
      .limit(20000),
    ...
    svc
      ...
      .select("id,hint,status")
      .order("created_at"),
  ]);

  const planMap = new Map(
    (plans.data ?? []).map((p) => [p.user_id, p]),
  );
  ...
  for (const c of customers.data ?? []) {
    ...
    if (
      !prev ||
      new Date(c.last_msg_at) > new Date(prev)
    ) {
      lastActive.set(c.user_id, c.last_msg_at);
    }
  }

  // Per-user message aggregates
  const msgCount = new Map<string, number>();

  for (const m of convos.data ?? []) {
    msgCount.set(
      m.user_id,
      (msgCount.get(m.user_id) ?? 0) + 1,
    );
  }
  ...
  return NextResponse.json({
    ...
    // The encrypted Gemini key itself is NEVER exposed.
    platform_keys: (platKeys.data ?? []).map((k) => ({
      id: k.id,
      hint: k.hint,
      status: k.status,
    })),
    ...
    paysettings: {
      ...
      pay_number: s?.pay_number ?? "",
      pay_instructions: s?.pay_instructions ?? "",
      pro_days: s?.pro_days ?? 30,
    },
